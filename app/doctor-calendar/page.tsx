...
export default function DoctorCalendar() {
  ...
  const handleEventDrop = async (args: EventDropArgs) => {
  try {
    ...
    if (response.ok) {
      ...
      setSuccessMessage(`Appointment with ${event.resource.patientName} has been rescheduled successfully!`);
  ...
  const handleRescheduleAppointment = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRescheduleDate('');
        setRescheduleTime('');
        setSuccessMessage(`Appointment with ${eventToReschedule.resource.patientName} has been rescheduled successfully!`);
        setShowSuccessMessage(true);
        setTimeout(() => setShowSuccessMessage(false), 3000);
      }
    } catch (error) {
  ...
  const eventStyleGetter = (event: CalendarEvent) => {
    return {
      style: {
        ...
        boxShadow: isDragging ? '0 8px 25px rgba(0,0,0,0.3)' : '0 2px 8px rgba(0,0,0,0.1)',
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-sm font-medium text-blue-800">
                  <strong>Drag & Drop:</strong> Click and drag appointments to reschedule them to different dates or times
                </p>
                <p className="text-xs text-blue-600 mt-1">
                  Appointments will automatically be marked as "rescheduled" when moved
                </p>
              </div>
            </div>
          </div>
          <div className="h-[600px]">
            <DragAndDropCalendar
              {...{
                ...
                step: 30,
                timeslots: 2,
                tooltipAccessor: (event: any) => `${event.resource.patientName} - ${event.resource.appointmentType}`,
